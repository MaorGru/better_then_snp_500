...
def mock_sp500_symbols():
    with patch(
        "app.clients.wikipedia_client.WikipediaClient.get_sp500_symbols",
...
def test_predict_endpoint_happy_flow():
    response = client.post(
        PREDICT_ENDPOINT, json={"symbol": "AAPL", "date": "2025-08-22"}
    )
...
def test_predict_endpoint_invalid_request():
    response = client.post(PREDICT_ENDPOINT, json={"symbol": "", "date": "2025-08-22"})
...
def test_predict_endpoint_symbol_not_in_sp500():
    response = client.post(
        PREDICT_ENDPOINT, json={"symbol": "CCC", "date": "2025-08-22"}
    )
